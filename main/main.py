...
from command import process_command, listen
...
def main(hotkey):
    ...
    try:
        ...
        with sr.Microphone() as source:
            ...
            if hotkey in read_config()[0]:
                p = Process(target=process_command, args=(command,))
                p.start()
            else:
                p = Process(target=listen, args=(command,))
                p.start()
...
if __name__ == '__main__':
    ...
    with keyboard.GlobalHotKeys({
             process_hotkey: lambda: main(process_hotkey),
            listen_hotkey: lambda: main(listen_hotkey)}) as h:
        h.join()
